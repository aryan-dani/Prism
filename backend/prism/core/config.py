...
# TITLE_MODEL gets its own keep-alive policy, separate from GEN_MODEL/EMBED_MODEL.
# Measured on this 8GB card (nvidia-smi): all three models held resident at
# once leaves ~100-200MB VRAM free, which is not enough slack for the KV
# cache to grow on longer conversations -- Windows silently falls back to
# system-RAM paging for the overflow, which is where the 50-180s latency
# spikes on multi-turn/anaphoric queries come from (verified: short
# single-turn calls stay in the 2-15s range on the same box when VRAM has
# headroom). A time-based short keep-alive (e.g. "2m") still loses the race
# under heavy session churn -- titling on turn 1 of every new session keeps
# resetting the timer before it ever idles out. "0" unloads immediately
# after each title call instead, guaranteeing GEN_MODEL always has the
# ~2GB back. TITLE_MODEL reloads fast (disk-cached) and generate_title_fast
# already has a 2.5s timeout + heuristic fallback, so an occasional slower
