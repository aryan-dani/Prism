# Prism — Architecture & Sourcing Decisions
...
- 768-dimensional embeddings, 8k token context — comfortably covers our
...
- Runs through the same Ollama runtime as generation, so there's one process
  to manage, one health check, one deployment story — versus standing up a
  ...
  `bge-small-en-v1.5` run on CPU via `sentence-transformers` — code is
...
- **Budget math**: nomic-embed-text's own weights are tiny; the real question
  ...
  simulated retrieval+generation turn) — see Section 3 for the outcome.

## 2. Vector store: Chroma (embedded/persistent) + BM25 sidecar

- **Chroma**, not a client-server vector DB (Qdrant/Milvus/Weaviate server
  mode, Pinecone, etc.) — no separate service to run, start, or keep alive
...
- **BM25 sidecar** (`rank_bm25`, pickled to `data/chroma/bm25_index.pkl`):
  ...
  token matches that these five domains actually hinge on — Kohler model
  numbers (`K-3901`), section numbers, and Finance's numeric thresholds
  (`₹25,000`, `Net 45`). Retrieval fuses dense + sparse results via
  reciprocal-rank fusion (`RRF_K = 60`, standard constant) rather than
  picking one. This is a retrieval-time fusion, not a second LLM call — no
...
(`eval/bench_models.py` → `eval/results/model_bench.md`):
...
1. **Finance exactness** (the deliberate ₹-threshold demo) is 2× better than
   Llama/Mistral on this pass — the single most important quality signal for
   this submission.
2. **Qwen3 8B** (`qwen3:8b`, pulled and benchmarked Sep 2026) matches
   ...
   same 12-question slice, but is ~3× slower and uses more VRAM — not a win
...
4. Schema adherence is 100% across candidates after the null→[] coercion in
...
— confirming the "no LLM call just to pick a domain" design works.
...
cheap vector math — zero additional LLM calls just to decide "which domain
...
- Confidence floor (`ROUTER_MIN_CONFIDENCE = 0.30`): below this, even the
  best-scoring domain isn't trusted — falls back to domain-agnostic
  retrieval across the whole collection.

## 5. Orchestration: hand-rolled state machine, not a framework

Chosen deliberately over LangGraph/LlamaIndex (see plan discussion) — the
agent's control flow (route → retrieve → answer → render, with clarify/
...
## 6. Domain sourcing: 3 real, 2 synthetic — and why
...
| Privacy | Real (`kohler.com/en/legal/privacy-policy`, CCPA page) | Legally required to be public; synthesizing a privacy policy would be actively counterproductive — the whole point of this domain is testing grounded, honest answers against a real legal document. |
| Legal/Compliance | Real (T&C, Prop 65, SDS, green-building disclosures, warranty pages) | Same reasoning as Privacy — Prop 65/SDS disclosures are legally mandated public documents, not goodwill content. |
...
| Finance | Synthetic (`data/synthetic/finance_policy.md`) | Same reasoning as HR. Structurally informed by the publicly-published AFOA "Sample Templates and Synopses of Financial Policies" reference PDF provided for the project. Deliberately contains unambiguous numeric thresholds (approval bands, per diem rates, PO requirements) as exactness-verification checkpoints for the agent — see `eval/golden.jsonl`. |
...
public internal Kohler document exists — but they're written to be
...
belong to Finance (e.g. WFH equipment allowance, reimbursement thresholds) —
...
exact widget content — title, markdown body, choice buttons, linked
PDFs/spreadsheets — with **zero nav/footer/boilerplate**, because that
...
video-only, warranty) before writing the parser — see `parse_assist.py`'s
...
Verified: bare `httpx`/PowerShell `Invoke-WebRequest` → 403; `curl_cffi` with
`impersonate="chrome"` → 200. `fetch.py` uses `curl_cffi` specifically for
...
**381 raw links → 359 unique article slugs** (5 cross-listed across
...
**No subset prioritization is needed — the full scrape is in scope and
...
silently dropped or faked — if it's still unreachable at final crawl time,
...
survives the cut; plus one line in the generation notes telling the model to
answer the follow-up rather than re-summarize. No change to routing, the
sticky domain, or the main retrieval path.
